...
begin
  enemies = File.open('E:/Games/Portal 2/Fix Repair/Tower of Trample 1.18.0.4/Tower of Trample 1.18.0.4/Enemies_decrypted.rvdata2', 'rb') { |f| Marshal.load(f) }
  count = 0
  enemies.compact.each do |e|
    name = e.instance_variable_get(:@name)
    if name && name =~ /Butterfly/i
      params = e.instance_variable_get(:@params)
      puts "#{e.instance_variable_get(:@id)}: #{name} - HP: #{params[0]} ATK: #{params[2]} DEF: #{params[3]}"
