...
begin
  data = load_with_dummy_classes("E:/Games/Portal 2/Fix Repair/GAME/TOT/Save01.rvdata2")
  actors = data[:actors]
  actor1 = actors.instance_variable_get(:@data)[1] # $game_actors[1]
  param_plus = actor1.instance_variable_get(:@param_plus)
  puts "Original Param Plus: #{param_plus.inspect}"
  
  param_plus[2] += 3000
  puts "New Param Plus: #{param_plus.inspect}"
  
  File.open("E:/Games/Portal 2/Fix Repair/GAME/TOT/Save01.rvdata2", "wb") do |f|
    Marshal.dump(data, f)
  end
  puts "Successfully patched Save01.rvdata2!"
rescue Exception => e
