...
- **Projects showcase** – Section redesigned per prompt with premium `DashboardCard` layout, live badges, metrics grid, key insights, and technical highlights, embedding all three Power BI dashboards inline.
...
- **Projects enhancements** – Layout is in place but still needs motion polish (magnetic hover, fade-ins), embed performance QA, and any supporting assets (thumbnails, copy tweaks) from 21st.dev references.
