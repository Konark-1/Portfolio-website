...
try {
    ...
    if (lcp && lcp.details && lcp.details.items) {
        lcp.details.items.forEach(item => {
            // LCP element items usually have a 'node' property or similar
            // Structure: { node: { snippet: '...', nodeLabel: '...' } }
            // Or flat items depending on version. Let's dump the whole item lightly.
            if (item.node) {
